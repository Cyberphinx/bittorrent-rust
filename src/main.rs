...
use bittorrent_rust::{decode::Decoder, encode::Encode, parse::Parse};
...
fn main() -> Result<()> {
    ...
    if command == "decode" {
        ...
        let torrent = Parse::parse_torrent_file(file_name)?;
        ...
        Encode::encode_file(file_path, announce_url, piece_length)?;
    } else {
